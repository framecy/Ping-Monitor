...
## [v2.6.0] - 2026-08-26 · 顶栏下线与卡片体系统一

### Changed
- **顶部栏整体下线**：所有页面顶部的白色标题卡移除，页面内容直达窗口顶部。全局控制重新归位——语言切换成为侧边栏底部用户行右侧的小胶囊（点击即切换）；全局监控开关移至监控页工具栏右上角（添加按钮右侧，两者等高）；Tailnet 状态胶囊移入侧边栏底部（仅启用 Tailscale 时显示）。
- **监控页工具栏升级**：「监控中主机」标题升为页面标题级字号，主机数量以灰色小字跟在标题后；`ToolbarRow` 组件新增 `count:` 参数支撑该样式。
- **侧边栏呼吸感**：导航行高从 28pt 增至 36pt，行距与分组间距同步放大。
- **网速页切换器归位**：接口/进程 切换器从悬浮卡改为页首工具行，选择经 `@AppStorage` 跨启动保留。
- **接口分组重设计**：展开分组不再整块罩灰——组头统一白底，层级靠缩进表达，成员行间发丝线分隔，选中接口改为内缩圆角浅蓝高亮，展开/收起带动画；PKT 列不再被卡片边缘裁切。
- **路由追踪地图成卡**：地图从全出血贴边改为与其他内容一致的圆角卡片，固定高度三档可切（小 200 / 中 300 / 大 440，右上角分段切换、跨启动记忆）；原地图与表格间的可拖分栏移除，整页单列滚动。
- **路由追踪工具卡内边距四边统一**为 16pt（原下边只有 8pt，视觉失衡）。
- **空态入卡**：路由追踪空态内容从裸放画布改为与内容体系一致的卡片。
- **无工具栏页面顶部留白修正**：`ScrollPage` 骨架为无工具栏页面补上 16pt 顶部留白，统计/服务/Tailscale/监控四页首卡不再几乎贴住窗口顶。
- **组件层演进**：`SegmentedSwitcher` 新增轨道底色参数（可置于白色卡片上）；`ToolbarRow` 标题升档并支持灰色计数。
- **设计规范文档同步**：CLAUDE.md / AGENTS.md 删除已过时的「地图全出血」例外条款；两轮 UI 审查报告（16 个问题的分类、根因与三层修复方案）收入 `archive/` 留档。

### Fixed
- **路由追踪工具行按钮被压扁**（回归修复）：输入框的 `layoutPriority` 抢占使「开始追踪」被挤成竖条、NSLookup 文字消失；按钮改用 `fixedSize` 永不压缩。
- **网速页英文串漏翻**：「Physical totals exclude tunnel bytes…」补齐中英文案。
- **编辑主机弹层规则行破损**（macOS 26）：系统 Form 会把占位符浮动到字段上方，定宽小字段将「如： P2P」顶出行外与开关行相撞；规则行重排为三行布局。
- **编辑弹层说明文字误用强调蓝**：看起来像可点链接，改回次级灰。

---

## [v2.5.0] - 2026-08-22 · Tailscale 集成成为真正的开关

### Changed
- **Tailscale 集成总闸全面生效**：此前的「Tailscale」设置项只隐藏界面入口 —— 应用启动仍会执行 `which tailscale` 探测、导入过的节点仍会升级为 `tailscale ping` 探测、已配置 OAuth 凭据时仍会轮询控制面 API。现在该开关是真正的功能闸门：关闭时不做 CLI 检测、不发任何控制面请求、Tailnet 清单停止轮询并清空、全部探测退回普通 `ping`；未安装 Tailscale 的机器上零进程派生、零报错。开关沿用原有设置项（默认关闭），此前已开启的用户升级后保持开启。
- **开关切换即时生效**：监控运行中开启/关闭集成，标准 ICMP 探测自动重启 —— 开启时升级为 tailscale 探测，关闭时退回普通 ping，无需重启应用。

### Added
- **设置页集成状态提示**：开关下方实时显示 CLI 检测结果（含可执行文件路径）或控制面凭据状态；安装 Tailscale 后重新打开设置页即自动重新检测，无需重启应用。

---

## [v2.4.1] - 2026-07-31 · 布局与网速统计修复

### Fixed
- **网速「全部接口」速率叠加**：选择全部接口时总速率数倍于实际，手动选中 en0 才准确。macOS 上同时存在十余个 utun 设备，链式封装（VPN 套 VPN、私密中继）使同一份载荷在多条 utun 上各记一次，隧道求和得到 21.3 MB/s 而物理链路只有 5.7 MB/s —— 隧道不可能比承载它的链路更快。现「全部接口」只报告承载默认路由的物理口速率（隧道流量本就封装其中，仍由隧道指标单独呈现）；隧道数值改取最忙的一条而非求和，总量与接口分组表头同步修正。
- **主机详情页卡片挤在左侧**：并排卡片使用 `GridItem(.adaptive(minimum:))`，而 `.adaptive` 的语义是「塞进尽可能多的列」—— 在约 1700pt 宽度下排出六列却只填两个，两张卡片被压在页面左三分之一。改用统计页已验证的按宽度切换 `Grid`/`GridRow` 方案。
- **主机详情页文本溢出**：质量分行（含 P95 与丢包率）不再被截断为省略号，改为换行展示；延迟统计四项指标改为按需缩放，「999.0 ms」不再折成两行。

### Changed
- **响应式栅格收敛**：容器宽度测量、两列断点与等高单元格抽取为 `ContainerWidthKey` / `.measureContainerWidth` / `.gridCell()` / `Theme.Layout.fitsTwoColumns`，统计页原有的私有实现已移除，两处页面共用同一套断点。
